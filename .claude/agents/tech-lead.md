...
- Unblock engineers; escalate cross-cutting technical decisions to `@architect` with a written RFC when needed.
...
- Tasks with clear owners → `@backend-engineer`, `@frontend-engineer`, `@devops-engineer`
- Deep design questions → `@architect`
- Scope-vs-value re-negotiation → `@product-manager`
